...
2. **Model config needs a refresh and has a foot-gun.** `appsettings.json` targets `gpt-realtime-1.5` (superseded by `gpt-realtime-2.1`, July 6, 2026) and `ChatModel: "gpt-5.4"` — which, under `VoicePipeline: "auto"`, silently selects the **legacy** pipeline because auto-detection checks for the substring `"realtime"` in the chat model name (`RobotAppOptions.cs:297-311`). See §3.
3. **Face tracking uses a hosted LLM as a per-frame detector** (`OpenAiHeadDetector` → `gpt-4o-mini` via `/v1/responses`, `detail:low`, ~5 Hz target). That's a network round-trip and billed vision request per frame for a job a local ONNX model does in <10 ms, free, offline. Swap it behind the existing `IHeadDetector` seam. See §2.3.
...
4. **Deduplicate the Responses API DTOs.** `ResponsesRequest`/`ResponsesInputItem`/content-part records exist twice (`OpenAiTransport.cs:918-943` and `OpenAiHeadDetector.cs:48-60`). One wire-model module.
5. **Wire or remove the dead `"ReachTether.Server"` HttpClient** (`Program.cs:81`) — every prior doc identifies this as the intended server seam; it currently has zero consumers.
...
**Sprint 1 — unblock the voice loop (subset of Phase 0; prerequisite for everything).**
...
**Sprint 2 — memory MVP (the interesting part).** Deliberately skips SignalR/video/sub-agents; the Mind starts as a boring little HTTP service:
