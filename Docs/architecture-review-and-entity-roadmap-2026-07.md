...
2. **Model config needs a refresh and has a foot-gun.** *(audit: config differs from what's described — `ChatModel` is now `"gpt-realtime-1.5"`, so `auto` currently resolves to the **realtime** pipeline; but the substring mechanism at `RobotAppOptions.cs:307-321` is unchanged and the model IDs are still stale. See §10.1.)* `appsettings.json` targets `gpt-realtime-1.5` (superseded by `gpt-realtime-2.1`, July 6, 2026), and under `VoicePipeline: "auto"` pipeline selection silently keys off the substring `"realtime"` in the chat model name. See §3.
...
*(audit: #1 and #4 confirmed still true on main as of 2026-07-18 — the beta realtime SDK and `LogResponsesApiBodies: true` are both unchanged. #2's premise changed: `ChatModel` is now `"gpt-realtime-1.5"` so `auto` picks realtime today, but the substring check (`RobotAppOptions.cs:307-321`) remains the foot-gun and the fix — explicit `VoicePipeline` — still applies. #3 describes the parked vision branch; main has no per-frame detector to replace. #5 still applies; note `FallbackChatModel` is now `"gpt-5-mini"` in config while code defaults remain stale.)*
...
3. **Config facts:** `ChatModel` is `"gpt-realtime-1.5"` (not `"gpt-5.4"`), so `auto` currently resolves to the realtime pipeline; the substring foot-gun moved to `RobotAppOptions.cs:307-321`. `FallbackChatModel` is `"gpt-5-mini"`. `Diagnostics:LogResponsesApiBodies` is still `true` (§2.2 #4 stands).
...
| Consolidation job (session-end + nightly, cheap model) | ⚠️ **Different shape** | No batch job. Per-turn fire-and-forget promotion (`MemoryPromotionService`): LLM fact extraction (`gpt-5-mini`→`gpt-5-nano`, strict JSON schema, confidence-gated) with regex fallback; dedup/supersede; rolling session summary; embedding per record |
...
5. **Model IDs predate the §3.2 matrix:** extraction `gpt-5-mini`/`gpt-5-nano`, smarty `gpt-5.4`, embeddings `text-embedding-3-small`, voice `gpt-realtime-1.5`. The Sprint-1 config refresh should now also cover the **server's** `appsettings.json`, which §8.1 didn't anticipate.
